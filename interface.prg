...
FUNC BUILD_RETSTATEMENT(TABLE_ALIAS)
   ODBC_TABLE = CURVAL('TABLE', TABLE_ALIAS)
   IF CURVAL('RETTYPE', TABLE_ALIAS) == 'INSERT'
      STATEMENT = 'INSERT INTO ' + ODBC_TABLE + ' ('
      STATEMENT = STATEMENT + ADD_RETODBC_FIELD(TABLE_ALIAS, 'IO', 1)
      STATEMENT = STATEMENT + ADD_RETODBC_FIELD(TABLE_ALIAS, 'TRACKNUM', 1)
      STATEMENT = STATEMENT + ADD_RETODBC_FIELD(TABLE_ALIAS, 'SERVCHRG', 0)
      STATEMENT = STATEMENT + ADD_RETODBC_FIELD(TABLE_ALIAS, 'PUBCHR', 0)
      STATEMENT = STATEMENT + ADD_RETODBC_FIELD(TABLE_ALIAS, 'DISCCHR', 0)
      ...
      STATEMENT = STATEMENT + ADD_RETBLANK(TABLE_ALIAS, 'PUBCHR', 0)
      STATEMENT = STATEMENT + ADD_RETBLANK(TABLE_ALIAS, 'DISCCHR', 0)
      STATEMENT = STATEMENT + ')'
   ELSE IF CURVAL('RETTYPE', TABLE_ALIAS) == 'UPDATE'
      STATEMENT = 'UPDATE ' + ODBC_TABLE + ' SET '
      STATEMENT = STATEMENT + ADD_UPDATEODBC_FIELD(TABLE_ALIAS, 'TRACKNUM', 1)
      STATEMENT = STATEMENT + ADD_UPDATEODBC_FIELD(TABLE_ALIAS, 'SERVCHRG', 0)
      STATEMENT = STATEMENT + ADD_UPDATEODBC_FIELD(TABLE_ALIAS, 'PUBCHR', 0)
      STATEMENT = STATEMENT + ADD_UPDATEODBC_FIELD(TABLE_ALIAS, 'DISCCHR', 0)
      STATEMENT = STATEMENT + ' WHERE '
      STATEMENT = STATEMENT + ADD_UPDATEODBC_FIELD(TABLE_ALIAS, 'IO', 0)
   ELSE
...
PROCEDURE BEFORE_PARCEL_DATAWRITE
   IF ODBC_CONNECT = .T.  && MAKE SURE CONNECT TO ODBC DATASOURCE
      IF INTERFACE_ON  && MEANS THE INTERFACE HAS TO BE ON IN ORDER TO CHECK SHIP CODES
         ...
         STATEMENT = STRTRAN(STATEMENT, "%PUBCHRG%", XTOTCHGVAR(1,4))
         STATEMENT = STRTRAN(STATEMENT, "%DISCCHRG%", XTOTCHGVAR(1,5))
         DO ODBC_EXECUTE IN INTERFACE WITH FILLINRETVALS()
